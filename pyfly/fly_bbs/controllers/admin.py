...
class CatalogsForm(form.Form):
    ...
    user_id = fields.SelectField('版主', widget=Select2Widget())
...
class CatalogsModelView(BaseModelView):
    ...
    def __get_user_id_choices(self, form):
        from fly_bbs.extensions import mongo
        form.user_id.choices = [(user['_id'], user['username']) for user in mongo.db.users.find()]

    def get_form(self):
        form = super(CatalogsModelView, self).get_form()
